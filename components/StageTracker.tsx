...
export default function StageTracker({ stages, currentStatus, sectionTitle = 'EXPERIMENTS & PIPELINE' }: StageTrackerProps) {
  ...
  return (
    <div className="glass-card rounded-2xl p-4 mb-6 transition-all duration-300">
      <div className="flex items-center justify-between mb-3 border-b border-[var(--border-color)] pb-2.5">
        ...
        <div>
          {currentStatus === 'SUCCESS' && (
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
              <CheckCircle2 className="w-3.5 h-3.5" /> SUCCESS
            </span>
          )}
          {currentStatus === 'AMBIGUOUS_NEEDS_INPUT' && (
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/30 animate-pulse">
              <AlertTriangle className="w-3.5 h-3.5" /> AMBIGUOUS_NEEDS_INPUT
            </span>
          )}
          {currentStatus === 'VALIDATION_FAILED' && (
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-rose-500/10 text-rose-400 border border-rose-500/30">
              <AlertCircle className="w-3.5 h-3.5" /> VALIDATION_FAILED
            </span>
          )}
          {currentStatus === 'LOADING' && (
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-blue-500/10 text-blue-400 border border-blue-500/30">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {stageDefinitions.map((def) => {
          ...
          let stateStyle = 'bg-[var(--bg-primary)] border-[var(--border-color)] text-[var(--text-secondary)]';
          let iconElement = <Clock className="w-4 h-4 opacity-50" />;
          ...
          if (matchingStep) {
            if (matchingStep.status === 'success') {
              ...
              stateStyle = 'bg-blue-950/20 border-blue-500/50 text-blue-200 shadow-lg';
              iconElement = <Loader2 className="w-4 h-4 text-[var(--accent-color)] animate-spin" />;
            }
          }
          ...
          return (
            ...
            >
              ...
              <p className="text-[11px] leading-tight text-[var(--text-primary)] opacity-90 line-clamp-2">
